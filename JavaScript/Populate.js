    ...
    products.forEach(product => {
        const productHTML = `
            <div class="group relative border border-zinc-200 p-3 rounded-lg" id="${product.id}">
                ...
                <button type="button" class="add-to-cart mt-3 w-full inline-flex items-center rounded-md justify-center bg-red-600 px-20 py-2.5 text-sm font-semibold text-white hover:bg-red-700" data-product-id="${product.id}" id="successButton" data-modal-target="successModal" data-modal-toggle="successModal">
            ...
            <div id="successModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                <div class="relative p-4 w-full max-w-md h-full md:h-auto ">
                    ...
                    <div class="relative p-4 text-center bg-stone-50 rounded-lg shadow sm:p-5">
                        <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-red-400 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="successModal">
    ...
    products.forEach(product => {
        if (product.type === 'Shorts') {
            ...
            <div class="group relative" id="${product.id}">
                ...
                <button type="button" class="add-to-cart mt-3 w-full inline-flex items-center rounded-md justify-center bg-red-600 px-20 py-2.5 text-sm font-semibold text-white hover:bg-red-700" data-product-id="${product.id}">
                    <svg class="-ms-2 me-2 h-6 w-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4h1.5L8 16m0 0h8m-8 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm8 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm.75-3H7.5M11 7H6.312M17 4v6m-3-3h6" />
                    </svg>
                    Add to Cart
...
document.addEventListener('DOMContentLoaded', () => {
    const showShortsButton = document.getElementById('ShowShortsButton');
    showShortsButton.addEventListener('click', (event) => {
    ...
    products.forEach(product => {
        if (product.type === 'Tshirt') {
            ...
            <div class="group relative" id="${product.id}">
                ...
                <button type="button" class="add-to-cart mt-3 w-full inline-flex items-center rounded-md justify-center bg-red-600 px-20 py-2.5 text-sm font-semibold text-white hover:bg-red-700" id="AddToCart">
                    <svg class="-ms-2 me-2 h-6 w-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4h1.5L8 16m0 0h8m-8 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm8 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm.75-3H7.5M11 7H6.312M17 4v6m-3-3h6" />
                    </svg>
                    Add to Cart
...
document.addEventListener('DOMContentLoaded', () => {
    const showTshirtsButton = document.getElementById('ShowTshirtsButton');
    showTshirtsButton.addEventListener('click', (event) => {
        event.preventDefault();
        ShowTees();
    });
});
